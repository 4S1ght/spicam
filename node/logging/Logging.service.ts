...
export default class LoggingService {
            ...
            this.winston = winston.createLogger({
                ...
                transports: [
                    new winston.transports.Console({
                        ...
                        level: this.config.settings.logging.level
                    }),
                    new winston.transports.DailyRotateFile({
                        filename:       path.join(this.config.settings.logging.folder, "%DATE%.log"),
                        auditFile:      path.join(this.config.settings.logging.folder, "audit.json"),
                        datePattern:    'YYYY-MM-DD',
                        format:         this.formats.file,
                        level:          this.config.settings.logging.level,
                        maxSize:        this.config.settings.logging.maxLogSize,
                        maxFiles:       this.config.settings.logging.maxLogFiles,
                        zippedArchive:  true,
                        createSymlink:  true,
    ...
    private levelsColored: Record<string, string> = {
        crit:   c.red('CRIT'),
        error:  c.red('ERRO'),
        warn:   c.yellow('WARN'), 
        notice: c.cyan('NOTE'),
